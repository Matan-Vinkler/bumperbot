...
SimpleTfKinematics::SimpleTfKinematics() : Node("simple_tf_kinematics"), x_increment_(0.05), last_x_(0.0), max_x_(5.0), rotation_counter_(0)
{
...
void SimpleTfKinematics::timerCallback()
{
    ...
    dynamic_transform_stamped_.transform.rotation.x = q[0];
    dynamic_transform_stamped_.transform.rotation.y = q[1];
    dynamic_transform_stamped_.transform.rotation.z = q[2];
    dynamic_transform_stamped_.transform.rotation.w = q[3];
...
void SimpleTfKinematics::getTransformCallback(const bumperbot_msgs::srv::GetTransform::Request::SharedPtr req, const bumperbot_msgs::srv::GetTransform::Response::SharedPtr res)
{
    RCLCPP_INFO_STREAM(get_logger(), "Requested transform: " << req->frame_id << " -> " << req->child_frame_id);

    auto requested_transform = geometry_msgs::msg::TransformStamped();
