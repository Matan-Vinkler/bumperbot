...
class AStarPlanner(Node):
    def __init__(self):
        ...
        self.map_sub_ = self.create_subscription(OccupancyGrid, "/costmap/costmap", self.mapCallback, map_qos)
        self.pose_sub_ = self.create_subscription(PoseStamped, "/goal_pose", self.goalCallback, 10)
        self.path_pub_ = self.create_publisher(Path, "/a_star_path", 10)
        self.map_pub_ = self.create_publisher(OccupancyGrid, "/a_star/visited_map", 10)
    ...
    def goalCallback(self, pose_msg: PoseStamped):
        ...
        try:
            map_to_base_tf = self.tf_buffer_.lookup_transform(self.map_.header.frame_id, "base_footprint", rclpyTime)
        except LookupException:
            self.get_logger().error("Could not transform from 'map' to 'base_footprint'!")
            return
        
        map_base_pose = Pose()
        map_base_pose.position.x = map_to_base_tf.transform.translation.x
        map_base_pose.position.y = map_to_base_tf.transform.translation.y
        map_base_pose.orientation = map_to_base_tf.transform.rotation
    ...
    def plan(self, start: Pose, goal: Pose) -> Path:
        ...
        while not pending_nodes.empty() and rclpy.ok():
            ...
            self.map_pub_.publish(self.visited_map_)
    ...
    def manhattan_distance(self, node1: GraphNode, node2: GraphNode) -> int:
