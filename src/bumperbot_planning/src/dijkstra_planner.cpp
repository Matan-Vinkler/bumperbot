...
namespace bumperbot_planning
{
    DijkstraPlanner::DijkstraPlanner() : Node("dijkstra_planner")
    {
        ...
        map_sub_ = create_subscription<nav_msgs::msg::OccupancyGrid>("/map", map_qos, std::bind(&DijkstraPlanner::mapCallback, this, _1));
        pose_sub_ = create_subscription<geometry_msgs::msg::PoseStamped>("/goal_pose", 10, std::bind(&DijkstraPlanner::goalCallback, this, _1));
    ...
    void DijkstraPlanner::mapCallback(const nav_msgs::msg::OccupancyGrid::SharedPtr map_msg)
    {
        map_ = map_msg;
    ...
    void DijkstraPlanner::goalCallback(const geometry_msgs::msg::PoseStamped& pose_msg)
    {
        if(!map_)
        {
    ...
    nav_msgs::msg::Path DijkstraPlanner::plan(const geometry_msgs::msg::Pose& start, const geometry_msgs::msg::Pose& goal)
    {
        ...
        while (!pending_nodes.empty() && rclcpp::ok())
        {
            ...
            if(active_node == world2Grid(goal))
            {
                break;
            }
            ...
            for(const auto& dir: explore_directions)
            {
                ...
                if(std::find(visited_nodes.begin(), visited_nodes.end(), new_node) == visited_nodes.end() && poseOnMap(new_node) && map_->data.at(pose2Cell(new_node)) == 0)
                {
            ...
            visited_map_.data.at(pose2Cell(active_node)) = 10;
