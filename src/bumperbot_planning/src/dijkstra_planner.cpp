...
{
    void DijkstraPlanner::configure(const rclcpp_lifecycle::LifecycleNode::WeakPtr& parent, std::string name, std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
    {
        node_ = parent.lock();
        name_ = name;
        tf_ = tf;
    ...
    nav_msgs::msg::Path DijkstraPlanner::createPlan(const geometry_msgs::msg::PoseStamped& start, const geometry_msgs::msg::PoseStamped& goal)
    {
        ...
        while (!pending_nodes.empty() && rclcpp::ok())
        {
            ...
            if(active_node == world2Grid(goal.pose))
            {
                break;
            }
            ...
            for(const auto& dir: explore_directions)
            {
                ...
                unsigned int current_cost = costmap_->getCost(new_node.x, new_node.y);
                if(std::find(visited_nodes.begin(), visited_nodes.end(), new_node) == visited_nodes.end() && poseOnMap(new_node) && current_cost < 99)
                {
    ...
    GraphNode DijkstraPlanner::world2Grid(const geometry_msgs::msg::Pose& pose)
    {
    ...
    geometry_msgs::msg::Pose DijkstraPlanner::grid2World(const GraphNode& grid)
    {
        geometry_msgs::msg::Pose pose;
        pose.position.x = grid.x * costmap_->getResolution() + costmap_->getOriginX();
        pose.position.y = grid.y * costmap_->getResolution() + costmap_->getOriginY();
    ...
    {
        return 0 <= node.x && node.x < static_cast<int>(costmap_->getSizeInCellsX()) && 0 <= node.y && node.y < static_cast<int>(costmap_->getSizeInCellsY());
    }

    unsigned int DijkstraPlanner::pose2Cell(const GraphNode& node)
    {
