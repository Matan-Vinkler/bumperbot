...
{
    void AStarPlanner::configure(const rclcpp_lifecycle::LifecycleNode::WeakPtr& parent, std::string name, std::shared_ptr<tf2_ros::Buffer> tf, std::shared_ptr<nav2_costmap_2d::Costmap2DROS> costmap_ros)
    {
        node_ = parent.lock();
        name_ = name;
        tf_ = tf;
...
#else
    nav_msgs::msg::Path AStarPlanner::createPlan(const geometry_msgs::msg::PoseStamped& start, const geometry_msgs::msg::PoseStamped& goal, std::function<bool()>)
#endif
    {
        ...
        while (!pending_nodes.empty() && rclcpp::ok())
        {
            ...
            for(const auto& dir: explore_directions)
            {
                ...
                int current_cost = costmap_->getCost(new_node.x, new_node.y);
